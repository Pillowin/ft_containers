...
class RBNode {
	...
	- 1}, [E_JUNCTION_HIG_R] = {P_JUNCTION_HIG_R, sizeof(P_JUNCTION_HIG_R) - 1},
			...
			static bool is_left_child(node *node) {
				return (!is_null_node(node->parent) && node->parent->left ==
	node);
			}
			static bool is_right_child(node *node) {
				return (!is_null_node(node->parent) && node->parent->right ==
	node);
			}
	...
	?  x->color : -1) << std::endl; return x == NULL || x->color == SENTINEL;
			...
			int	print_connect(t_buf *b, int *printed)
			{
				...
				{
					ret = buffer_connect_leaf(b, 1, tree_dsp.pos - print,
	&print); tmp = left->tree_dsp.block_width + left->tree_dsp.block_delta;
				}
				if (!ret && !is_null_node(right))
				{
					tmp = right->tree_dsp.pos + right->tree_dsp.block_delta +
	tmp
			...
			int	print_label(t_buf *b, int *printed)
			{
				...
				ret = buffer_append_mul(b, E_PAD, tree_dsp.label_delta)
					...
					|| buffer_append_raw(b, color == RED ? "\033[31m" :
	"\033[34m", sizeof("\033[31m") - 1)
	...
	tree_dsp.label_width - WORD_ADDED_WIDTH - tree_dsp.trunc)
			...
			int	layer_print_specific(t_buf *b, int (node::*layer_fn)(t_buf *b,
	int *printed), int *ret)
			{
				int		print;
	...
	tree_dsp.block_delta);
			...
			int	layer_print_tree(int max_depth, int fd, int pretty)
			{
				...
				while (!ret && max_depth >= 0)
				{
					...
					{
						if (!ret)
							layer_print_specific(&b, &node::print_connect,
	&ret); if (!ret) ret = buffer_trim_to_newline(&b);
					}
				}
				if (!ret)
			...
			void	label_adjust()
			{
				tree_dsp.label_delta = tree_dsp.pos - tree_dsp.label_width / 2 -
	1; if (tree_dsp.label_delta < 0)
				{
			...
			{
				tree_dsp.block_width = std::max(tree_dsp.lwidth +
	tree_dsp.rwidth + 1, tree_dsp.label_width); if (depth_left && depth_right) {
	t_tree_display *l = &left->tree_dsp; t_tree_display *r = &right->tree_dsp;
	const int	lpos = l->block_delta + std::max(l->pos + 1, l->pos +
	(l->label_width - tree_dsp.label_width) / 2); const int	rpos =
	l->block_width + l->block_delta
	+ r->block_delta + std::min(r->pos - 1, r->pos + (r->label_width -
	tree_dsp.label_width) / 2); tree_dsp.pos = (lpos + rpos) / 2; } else if
	(depth_left){ t_tree_display *l = &left->tree_dsp; tree_dsp.pos =
	l->block_delta
	+ std::max(l->pos + 1, l->pos + (l->label_width - tree_dsp.label_width) /
	2); if (tree_dsp.lwidth > tree_dsp.label_width)
						--tree_dsp.block_width;
					tree_dsp.rwidth = tree_dsp.block_width - tree_dsp.lwidth;
				} else if (depth_right) {
					t_tree_display *r = &right->tree_dsp;
					const int	rpos = r->block_delta + std::min(r->pos - 1,
	r->pos
	...
	depth_right); tree->pos_calc(depth_left, depth_right); tree->label_adjust();
			...
			static int	_print_tree(node *root, int pretty, int fd)
			{
				...
				{
					if (pretty)
						dprintf(fd, "\033[31mCannot print a NULL
	tree\033[0m\n"); else dprintf(fd, "Cannot print a NULL tree\n"); return (0);
				}
